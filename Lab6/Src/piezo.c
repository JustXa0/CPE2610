...
void Play(uint16_t freq, uint16_t time) {
    ...
    period /= 2;
    tim3->PSC = TIM_1US_PSC; // Assuming this gives 1µs per tick
    tim3->ARR = period - 1;
    tim3->CCR1 = period - 1; // Toggle in the middle of period

    // Configure TIM3 in Output Compare Toggle Mode
    tim3->CCMR1 &= ~(0b111 << 4);  // Clear OC1M bits
    tim3->CCMR1 |= (0b011 << 4);   // Set OC1M to Toggle Mode (011)
    tim3->CCMR1 |= (1 << 3);       // Enable Output Compare Preload (OC1PE = 1)

    // Enable TIM3 CH1 Output
    tim3->CCER |= (1 << 0);  // Enable output on CH1
    tim3->CCER &= ~(1 << 1); // Ensure active high output
